...
def main(train_data_file, predict_data_file, summarized_sentences, w2v_model, testing, save_file_directory="",
    ...
    if testing:
        summarized_sentences = summarized_sentences[:len(train_data_file)]
    ...
    if train_flag_dict[NOVEL_FLAG]:
        if train_new:
            novel_model_dict, novel_history_dict, w2v_result_dict = lstm_main(summarized_sentences=summarized_sentences,
                                                                              predicted_data=predict_df,
                                                                              truth_dictionary=truth_dictionary,
                                                                              w2v_model=w2v_model, testing=testing,
                                                                              use_w2v=False)
...
if __name__ == "__main__":
    ...
    EXPT_NAME = ""
    feature_dictionary = {GAZETTE_FLAG: 0,
                          W2V_FLAG: 0,
                          NOVEL_FLAG: 0,
                          LDA_FLAG: 1,
                          LSI_FLAG: 1,
    ...
    if train_new:
        EXPT_NAME = "MY_EXPT_NAME_" + time.strftime(FILE_NAME_STRING_FORMATING)
        ...
        main(train_data_file=SAMPLE_DATA_FILE, predict_data_file=PREDICT_DATA_FILE,
             summarized_sentences=summarized_sentence_data,
             w2v_model=sample_model, testing=True, save_file_directory=SAVE_FILE_PATH, train_new=False,
             train_flag_dict=feature_dictionary)
        ...
        main(train_data_file=TRAIN_DATA_FILE, predict_data_file=PREDICT_DATA_FILE,
             summarized_sentences=summarized_sentence_data,
             w2v_model=real_model, testing=False, save_file_directory=SAVE_FILE_PATH, train_new=False,
             train_flag_dict=feature_dictionary)
